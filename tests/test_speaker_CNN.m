...
for i=1:size(files,1)
	...
	if !exist('train_x')
        ...
        train_y = zeros(2, usable) + label;
	    clear mfccs
	else
	    %Repeat speaker information until it is long as 'usable'
		times = floor((usable*winsize)/size(mfccs,2));
		mfccs = repmat(mfccs, 1, times);
		reshaped = reshape(mfccs(:, 1:usable*winsize), mfccnum, winsize, []);
	    clear mfccs	
	    train_x = cat(3, train_x, reshaped);
        clear reshaped
	    train_y = cat(2, train_y, zeros(2, usable) + label);
	end
       clear mfccs	
end
